...
def run(query_limit, topic_to_search):
    index = queryingtools.load_index('/scratch/cluster/dnelson/ir_proj/bm25_abstract_index', 'abstract')
...
def load_data(xml_file):
    ...
    searchable_topics = [(topic.attrs['number'], " ".join(topic.description.contents + [topic.attrs['type']])) for topic in topics]
...
if __name__ == '__main__':
    ...
    result_file = 'abs_query' + sys.argv[1] + '_lim' + sys.argv[2] + '_out.txt'
