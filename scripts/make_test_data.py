...
def make_truth(year=2014):
    ...
    # makes a dictionary of dictionaries, with a default value of -1.0
    truth_by_id = defaultdict(lambda: defaultdict(lambda: -1.0))
    by_topic = defaultdict(lambda: defaultdict(lambda: -1.0))
    known_ids = set()
